<template>
  <div class="container">
    <scroll-container
      ...
      threshold: 0.55, // 触发动画的阈值，默认是0.55
      exitAnim: false // 是否添加退出视口的动画
    }"
        class="slide-swiper" :max-width="1088">

      <div class="swiper-button-container">
        <h1 class="item-module-h1">{{title}}</h1>

<!--
        <div class="goto-select-box">
          <p>观看特别活动</p>
...
-->
      ...
      <div  ref="slideContainer" class="video-slide-container"
        ...
        <div class="swiper-item"
          ...
          <t-image  fit="cover" class="item-img" :src="item.imgUrl">
          </t-image>

          <div class="intro-text-box">
            <p :style="{color: item?.color}">{{item?.title1}}</p>
            <h1 :style="{color: item?.color}">{{item?.title2}}</h1>
          </div>
        </div>
      </div>
...
function gotoTargetPath(item) {
  router.push({path:localePath(item.path)})
}
