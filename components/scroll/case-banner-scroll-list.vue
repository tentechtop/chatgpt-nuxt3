<template>
  <div   class="component-container">
    <scroll-container  class="slide-swiper" :max-width="maxWidth">


      <div ref="slideContainer" class="slide-container-box"
        ...
        <div class="swiper-item"
          ...
          <template v-if="item.sourceType === 'img'">
            <img   class="item-box" :src="item.imgUrl"/>
          </template>

          <template v-if="item.sourceType === 'video'">
            <video-player class="item-box"  :is-selected="item.isSelected" :is-playing="item.isPlay" :src="item.videoUrl" :poster="item.videoUrl+'?x-oss-process=video/snapshot,t_1000,m_fast'"></video-player>
          </template>
      ...
      <div v-if="videoListData.length >1" class="swiper-button-dot-container">
...
import VideoPlayer from "~/components/scroll/video/video-player.vue";
...
const handleResize = () => {
  ...
  if (window.innerWidth>780){
    itemWidth.value = Math.min(Math.max(originalItemWidth, 300), window.innerWidth)
    marginRight.value = 16
    aspectRatioInfo.value = props.aspectRatio
  }else {
    itemWidth.value = Math.min(Math.max(originalItemWidth, 300), window.innerWidth);
    marginRight.value = 16
    aspectRatioInfo.value = 100/65
  }
}
